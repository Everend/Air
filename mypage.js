window.onload = function(){
	...
	countDown = setInterval(function(){
		second.innerHTML--;
		second.innerHTML == 0 && (msg.style.display = 'none',clearInterval(countDown));
	},1000)
	...
	for(var m=0;m<picture.length;m++){
		...
		document.onmousemove = function(event){			
			...
			var boxX = event.clientX - clickX;
			var boxY = event.clientY - clickY;
			var maxwidth = part[6].offsetWidth - picture[rank].offsetWidth;
			var maxheight = part[6].offsetHeight - picture[rank].offsetHeight;
			boxX = boxX < 0? 0:boxX;
			boxX = boxX > maxwidth? maxwidth:boxX;
			boxY = boxY < 70? 70:boxY;
			boxY = boxY > maxheight? maxheight:boxY;			
			picture[rank].style.left = boxX + "px";
			picture[rank].style.top = boxY + "px";
			seat[rank].push({x:boxX, y:boxY});
